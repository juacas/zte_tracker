...
from .const import DEFAULT_HOST, DEFAULT_PASSWORD, DEFAULT_USERNAME, DOMAIN
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        data_schema = vol.Schema(
            {
                vol.Required(CONF_HOST, default=defaults.get(CONF_HOST, DEFAULT_HOST)): cv.string,
                vol.Required(CONF_USERNAME, default=defaults.get(CONF_USERNAME, DEFAULT_USERNAME)): cv.string,
                vol.Required(CONF_PASSWORD, default=defaults.get(CONF_PASSWORD, DEFAULT_PASSWORD)): cv.string,
                vol.Required(CONF_MODEL, default=defaults.get(CONF_MODEL, "F6640")): vol.In(zteClient.get_models()),
            }
        )
